...
(defun wonima-project-consult-ripgrep ()
  "Like `consult-ripgrep', but prompt a project to use if currently not in one."
  (interactive)
...
(with-eval-after-load 'project
  (keymap-substitute project-prefix-map
                     #'project-find-regexp
                     #'wonima-project-consult-ripgrep)
  (defvar project-switch-commands)
  ;; not use the in-place/destructive `cl-nsubstitute-if'
  ;; to keep "Original value was" of docstring correct
  (setopt project-switch-commands
          (cl-substitute-if (list #'wonima-project-consult-ripgrep "Find regexp")
                            (lambda (item) (eq (car item) #'project-find-regexp))
                            project-switch-commands)))

(with-eval-after-load 'consult
  (eval-when-compile (require 'consult)) ;; let the compiler know macro `consult-customize'
  (declare-function consult--customize-put "consult")
  (consult-customize
   consult-fd consult-find :state (consult--file-preview)))
